...
# Subject: "Your receipt from Anthropic, PBC #2009-2705-8406"
_QUERY = 'from:anthropic.com subject:"Your receipt from Anthropic" has:attachment filename:pdf'
_INVOICE_ID_RE = re.compile(r"#([\d-]+)")


class AnthropicMailProvider(InvoiceProvider):
    name = "anthropic_mail"
    ...
    def list_invoices(self, since: date) -> list[Invoice]:
        ...
        for msg_ref in messages:
            try:
                ...
                log.warning("AnthropicMail: skipping message %s: %s", msg_ref["id"], e)

        log.info("AnthropicMail: %d invoice(s) since %s", len(invoices), since)
        return invoices
    ...
    def fetch_pdf(self, invoice: Invoice, dest_dir: Path) -> Path:
        try:
            ...
            dest = dest_dir / f"anthropic_{invoice.invoice_id}_{invoice.issue_date}.pdf"
