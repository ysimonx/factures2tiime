...
# Subject: "Envoi de la facture ..."
_QUERY = 'from:contact@certigna.com subject:"Envoi de la facture" has:attachment filename:pdf'


class CertignaMailProvider(InvoiceProvider):
    name = "certigna_mail"
    ...
    def list_invoices(self, since: date) -> list[Invoice]:
        ...
        for msg_ref in messages:
            try:
                ...
                log.warning("Certignamail: skipping message %s: %s", msg_ref["id"], e)

        log.info("Certignamail: %d invoice(s) since %s", len(invoices), since)
        return invoices
    ...
    def fetch_pdf(self, invoice: Invoice, dest_dir: Path) -> Path:
        try:
            ...
            dest = dest_dir / f"certigna_{invoice.invoice_id}_{invoice.issue_date}.pdf"
