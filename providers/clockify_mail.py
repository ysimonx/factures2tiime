...
# Clockify (CAKE.com) receipts are sent from receipt@mail.cake.com
_QUERY = 'from:receipt@mail.cake.com has:attachment filename:pdf'


class ClockifyMailProvider(InvoiceProvider):
    name = "clockify_mail"
    ...
    def list_invoices(self, since: date) -> list[Invoice]:
        ...
        for msg_ref in messages:
            try:
                ...
                log.warning("ClockifyMail: skipping message %s: %s", msg_ref["id"], e)

        log.info("ClockifyMail: %d invoice(s) since %s", len(invoices), since)
        return invoices
    ...
    def fetch_pdf(self, invoice: Invoice, dest_dir: Path) -> Path:
        try:
            ...
            part = next(
                (p for p in parts if p["filename"].startswith("Invoice-")),
                parts[0],
            )
            pdf_bytes = gmail_base.get_attachment_bytes(msg_id, part["attachment_id"])
            dest = dest_dir / f"clockify_{invoice.invoice_id}_{invoice.issue_date}.pdf"
