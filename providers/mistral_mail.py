...
# Subject: "Votre facture nº MSTRL-API-812453-001 de Mistral AI SAS"
_QUERY = 'from:mistral.ai subject:"Votre facture" has:attachment filename:pdf'
_INVOICE_ID_RE = re.compile(r"\b(MSTRL-[A-Z]+-\d+-\d+)\b")


class MistralMailProvider(InvoiceProvider):
    name = "mistral_mail"
    ...
    def list_invoices(self, since: date) -> list[Invoice]:
        ...
        for msg_ref in messages:
            try:
                ...
                log.warning("MistralMail: skipping message %s: %s", msg_ref["id"], e)

        log.info("MistralMail: %d invoice(s) since %s", len(invoices), since)
        return invoices
    ...
    def fetch_pdf(self, invoice: Invoice, dest_dir: Path) -> Path:
        try:
            ...
            dest = dest_dir / f"mistral_{invoice.invoice_id}_{invoice.issue_date}.pdf"
